...
In 2020, led by [Daniel Kunin](http://daniel-kunin.com/), [Aran Nayebi](https://anayebi.github.io/), and [Javier Sagastuy-Brena](https://www.javiersagastuy.com/), we published at ICML a follow up paper formalizing the link between our linear autoencoder results and the neural plausibility of backpropagation.  This work identified two routes (local and non-local) towards the discovery of neural implementations for credit assignment without weight symmetry.  In particular, this work considers the credit assignment problem for neural networks as a layer-wise regularization problem between a set of forward and backward weights. Different regularization functions constrain the relationship between the forward and backward weights, giving rise to different learning rules.  The regularization function is built from a set of simple, biologically motivated primitives with strong geometric interpretations. One of the local learning rules presented in this work is termed *Information Alignment*, because in a specific setting, the update to the backward weights is proportional to the gradient with respect to the decoder of a quadratically regularized linear autoencoder.  We found through a Bayesian search of the space of local learning rules, that a version of Information Alignment yields competitive performance with backpropagation even for large-scale learning scenarios, e.g. ImageNet categorization with deep convolutional networks. Check out our [ICML paper](http://proceedings.mlr.press/v119/kunin20a/kunin20a.pdf) and [presentation](https://slideslive.com/38928436/two-routes-to-scalable-credit-assignment-without-weight-symmetry?ref=speaker-22461-latest) for more information.
